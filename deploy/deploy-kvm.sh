#!/usr/bin/env bash
# TDM ECR PPF landing page -> Profitcast KVM (root@187.127.149.216)
#
...
DOCROOT="/var/www/ppf.lp.thedetailingmafiaecr.com"
LIVE_URL="https://ppf.lp.thedetailingmafiaecr.com"
PREVIEW_URL="https://tdmecr-ppf-preview.187.127.149.216.nip.io"
MODE="${1:-deploy}"
...
# excludes: this repo also holds README.md, audit-report.md, content/, deploy/
# and a 1 MB zip, none of which belong on a public server. tdm-ecr-logo.png is
...
case "$MODE" in
  ...
  deploy)
    ...
    ship_files | tar -czf - -T - | ssh_kvm "set -e; D=$DOCROOT
      T=\$(mktemp -d /var/www/.tdmecr-ppf-new.XXXX)
